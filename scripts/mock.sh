...
up() {
    ...
    (cd "$REPO_ROOT" && swift build --product TBDApp && swift build --product TBDDaemon) 2>&1 | tail -6
...
case "$cmd" in
    ...
    down)    down ;;
    shot)    shot "${1:-}" ;;
    restart) (cd "$REPO_ROOT" && swift build --product TBDApp && swift build --product TBDDaemon) 2>&1 | tail -6; down; up "${1:-default}" ;;
